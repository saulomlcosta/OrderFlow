...
test('customer recovers an active checkout from the account area', async ({ page }) => {
  const productName = `Account Keyboard ${Date.now()}`;
  ...
  await expect(page.getByRole('heading', { name: 'My checkouts' })).toBeVisible();
  await expect(page.getByText(productName)).toBeVisible();
  await page.getByRole('button', { name: 'Complete purchase' }).click();

  await expect(page.getByText('Checkout completed. Your order is now available below.')).toBeVisible();
  await expect(page.locator('.orders-panel').getByText(productName)).toBeVisible();
});
